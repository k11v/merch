...
После сдачи задания было желание и время сделать рефакторинг, написать тесты и в целом довести чеклист до завершения. Результат этого блока работы представлен в теге [0.2.0](https://github.com/k11v/merch/releases/tag/0.2.0).
...
* [x] Проведённое нагрузочное тестирование.
...
2. Запустите сервер и его зависимости вместе с профилем test.
   ...
   с тестовыми пользователями и токенами аутентификации.
...
3. Скопируйте файлы с тестовыми пользователями и токенами аутентификации.
...
4. Запустите нагрузочное тестирование, указав пути к скопированным файлам.
   ...
   Полученные результаты можно сравнить с результатами, которые были получены ранее.
...
UUID в качестве первичных ключей имеют множество преимуществ, например, его никогда не нужно изменять, его может легко сгенерировать и сервер, и клиент, он позволяет безболезненно сливать несколько таблиц в одну. Всё это даёт гибкость в использовании базы данных по сравнению с альтернативами (столбец id типа serial, столбец username типа text и другие).
...
Там, где абстракции могли принести пользу, они появлялись.

Операции с базой данных инкапсулировались в отдельные функции (например, getUserByUsername и getItemByName),
